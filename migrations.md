...
This strategy solves one important aspect of the *parallel versions problem*. In particular, it deals with the situation where an old app writes old documents into the system. But what if new documents get synced to an older application? So far, we have only taken Progressive Web Apps into consideration, but if we are serious about conquering that market we better build some native apps as well. So what if someone uses the iPhone app but does not update it immediately every time a new verion is available? Then there is an old app in a new system and it is confronted with newer documents generated by other, updated clients. This could break the old app or, if it chooses to ignore newer documents, lead to inconsistent or missing data. We cannot properly support older apps with continuous server-side migrations.
...
As a general migration strategy, this approach looks very promising indeed. It would allow us to write adapters for each API version that could migrate documents on the fly, up and down. However, we cannot pursue this path further at this point because CouchDB does not provide any hooks that we could use to insert our adapters into the dataflow. This would have serious consequences for many aspects of the system including the replication mechanism. Since this is not an option, let's not have this discussion right now and instead focus on what is feasible.

#### Client-side transactional migrations

We have found the server-side transactional migration to run into trouble because of different client versions that operate in the system. But perhaps we were throwing out the baby with the bathwater and transactional migrations would actually work if only we ran them directly on the clients. In this setup clients would continue to work with old documents but once they get updated they first run a migration on their local database and start working with new data.

This approach has several drawbacks. A first problem is that we do not get rid of



This approach would indeed prevent clients to send old data back into an updated system, but it as well has some problems of its own. First of all it would 

TBD: outlook to future deterministic ids
deterministic revesion ids

edge case: continuous migration of old docs necessary
